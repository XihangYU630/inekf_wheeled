...
for i=4:size(data_mat,1)
    ...
    if data_mat.VarName1(i) == "p"
        ...
        gt.position = [gt.position; data_mat.X(i) data_mat.Y(i) data_mat.Z(i)];
        gt.linear_velocity = [gt.linear_velocity; data_mat.v_x(i) data_mat.v_y(i) data_mat.v_z(i)];
        gt.camera_orientation = [gt.camera_orientation; quat2eul(temp)];
        gt.camera_position = [gt.camera_position; data_mat.X(i) data_mat.Y(i) data_mat.Z(i)];
    end
...
for i=1:length(results.position(:,1))
    ...
    test2 = [test2;(R_temp*results.position(i,:)')'];
end
test(53:68,:) = [];
plot3(orbslam(:,2),orbslam(:,3),orbslam(:,4))
plot3(test2(:,1),test2(:,2),test2(:,3),'b');

...
state.cov_w = 0.1*diag(ones(21,1));
state.cov_nf = diag([5 5 1]);
...
for i=2:length(time)
    ...
    %% Update Results
    results.orientation = [results.orientation; rotm2eul(state.R)];
    ...
    %% Update Ground Truth
    gt.orientation = [gt.orientation; camera.Orientation.X(index_camera) camera.Orientation.Y(index_camera) camera.Orientation.Z(index_camera)];
    gt.position = [gt.position; camera.Position.X(index_camera) camera.Position.Y(index_camera) camera.Position.Z(index_camera)];
    gt.linear_velocity = [gt.linear_velocity; camera.Velocity.Linear.X(index_camera) camera.Velocity.Linear.Y(index_camera) camera.Velocity.Linear.Z(index_camera)];
    gt.bias_w = [gt.bias_w; state.bw'];
    gt.bias_a = [gt.bias_a; state.ba'];
    gt.camera_orientation = [gt.camera_orientation; camera.Orientation.X(index_camera) camera.Orientation.Y(index_camera) camera.Orientation.Z(index_camera)];
    gt.camera_position = [gt.camera_position; camera.Position.X(index_camera) camera.Position.Y(index_camera) camera.Position.Z(index_camera)];
    gt.camera_position = [gt.camera_position; camera.Velocity.Linear.X(index_camera) camera.Velocity.Linear.Y(index_camera) camera.Velocity.Linear.Z(index_camera)];
end
%% Plotting
...
plot3(camera.Position.X,camera.Position.Y,camera.Position.Z,'black')

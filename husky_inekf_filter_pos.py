...
Visual_dom = pd.read_csv("Camera_odometry_corr_parking.csv")
...
Left_wheel_ang = Encoder.iloc[:, 4]
Right_wheel_ang = Encoder.iloc[:, 3]
...
class System:
    ...
    def choose_measurement(self, i):
        ...
        idx_3, t3 = find_nearest(Sec_camera_pos_total, t)
        threshold = 0.005
        if np.abs(t1 - t) < threshold or np.abs(t3-t) < threshold:
            if np.abs(t1 - t) < np.abs(t3 - t):
    ...
    def update_measurement(self, flag, idx, i):
        ...
        if(flag == "encoder"):
            ...
            self.y_pos = np.array([p_x_visdom[idx], p_y_visdom[idx], p_z_visdom[idx], 0, 1])

def plot(v, p, b_w, b_a, R_quat, p_c, R_c_quat, y_encoder_vel, y_encoder_vel_z, t):

    fig, axs = plt.subplots(4, 3, figsize=(15, 10))
    axs[0, 0].plot(t, v[:, 0])
    # axs[0, 0].plot(Sec_visdom_total[0:400], v_x_visdom[0:400])
    # axs[0, 0].plot(t, y_encoder_vel[:, 0])
    axs[0, 0].set_title("v_x")
    axs[0, 1].plot(t, v[:, 1])
    # axs[0, 1].plot(Sec_visdom_total[0:400], v_y_visdom[0:400])
    # axs[0, 1].plot(t, y_encoder_vel[:, 1])
    axs[0, 1].set_title("v_y")
    axs[0, 2].plot(t, v[:, 2])
    # axs[0, 2].plot(Sec_visdom_total[0:400], v_z_visdom[0:400])
    ...
    axs[1, 0].plot(t, p[:, 0])
    axs[1, 0].plot(Sec_camera_pos_total[0:1000], p_x_visdom[0:1000])
    axs[1, 0].set_title("p_x")
    axs[1, 1].plot(t, p[:, 1])
    axs[1, 1].plot(Sec_camera_pos_total[0:1000], p_y_visdom[0:1000])
    axs[1, 1].set_title("p_y")
    axs[1, 2].plot(t, p[:, 2])
    axs[1, 2].plot(Sec_camera_pos_total[0:1000], p_z_visdom[0:1000])
    ...
    axs[0, 0].plot(Sec_camera_pos_total[0:1000], q_x_visdom[0:1000])
    axs[0, 0].set_title("R_quat_x")
    axs[0, 1].plot(t, R_quat[:, 1])
    axs[0, 1].plot(Sec_camera_pos_total[0:1000], q_y_visdom[0:1000])
    axs[0, 1].set_title("R_quat_y")
    axs[0, 2].plot(t, R_quat[:, 2])
    axs[0, 2].plot(Sec_camera_pos_total[0:1000], q_z_visdom[0:1000])
    axs[0, 2].set_title("R_quat_z")
    axs[0, 3].plot(t, R_quat[:, 3])
    axs[0, 3].plot(Sec_camera_pos_total[0:1000], q_w_visdom[0:1000])
